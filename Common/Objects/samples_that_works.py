...
class TopicSample(Sample):
    ...
    def ApplyDocumentCutoff(self):
        ...
        print("Dataframe created success!!")
        ...
        print("Len of unknown list", len(unknown_list))
        unknown_df = pd.DataFrame()
        # unknown_df = document_topic_prob_df[document_topic_prob_df.index.isin(unknown_list)]
#
        # unknown_df = document_topic_prob_df.reset_index()[document_topic_prob_df.reset_index()[document_topic_prob_df.index.name].isin(unknown_list)]
...
class NMFSample(TopicSample):
    ...
    def GenerateFinish(self, result, dataset, current_workspace):
        ...
        for i in range(self.num_topics):
            ...
            print("parts: ", self.parts_dict[topic_num])
        self.parts_dict['unknown'] = TopicUnknownPart(self, 'unknown', [], dataset)
...
class BertopicSample(TopicSample):
    ...
    def GenerateStart(self, notify_window, current_workspace_path, start_dt):
        ...
        self.training_thread = SamplesThreads.BertopicTrainingThread(notify_window,
                                                                   ...
                                                                   self.tokensets,
                                                                   self.num_topics)
        print("new changed?:", self.num_topics)
        logger.info("Finished")
    ...
    def GenerateFinish(self, result, dataset, current_workspace):
        ...
        # print("result", result['document_topic_prob'])
...
class TopicMergedPart(ModelMergedPart):
    ...
    def GetTopicKeywordsList(self):
        ...
        print("Keywords_sorted:",keywords_sorted )
...
class BertopicTopicPart(TopicPart):
    ...
    def word_num(self, value):
        ...
        if self.key in topic_words_dict:
            word_prob_list = topic_words_dict[self.key]
            sorted_word_probs = sorted(word_prob_list, key=lambda x: x[1], reverse=True)
            truncated_word_probs = sorted_word_probs[:value]
            self.word_list = truncated_word_probs
            print(self.word_list)
        else:
            logger.warning(f"Topic with key {self.key} not found in the topic model.")
